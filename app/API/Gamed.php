...
class Gamed
{
    ...
    public function hex2int($value)
    {
        ...
        $value = $value[3] . $value[2] . $value[1] . $value['Server'];
        $value = hexdec($value);
        return $value;
    }
    ...
    {
        return $this->SendToSocket($data, $_ENV['GAMEDBD_PORT']);
    }
    public function SendToDelivery($data)
    {
        return $this->SendToSocket($data, $_ENV['GDELIVERYD_PORT'], true);
    }
    public function SendToProvider($data)
    {
        return $this->SendToSocket($data, $_ENV['GACD_PORT']);
    }
    public function SendToSocket($data, $port, $RecvAfterSend = false, $buf = null)
    {
        if (@fsockopen($_ENV['SERVER_HOST'], $port, $errCode, $errStr, 1)) {
            $sock = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
            socket_connect($sock, $_ENV['SERVER_HOST'], $port);
            if ($_ENV['S_BLOCK']) {
            ...
            switch ($_ENV['S_READTYPE']) {
                case 1:
                    socket_recv($sock, $buf, $_ENV['MAX_BUFFER'], 0);
                    break;
                case 2:
